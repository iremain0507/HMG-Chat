...
import { TEST_DATABASE_URL } from "./src/__tests__/integration/test-database";
...
export default defineConfig({
  test: {
    ...
    // 통합테스트는 dev DB 가 아니라 별도 test DB(wchat_test)에만 접속·리셋한다.
    //   dev 데이터 소실 방지. TEST_DATABASE_URL 로 override 가능.
    env: { DATABASE_URL: TEST_DATABASE_URL },
    // 매 실행 전 test DB 리셋 + 재마이그레이션 (편집한 마이그레이션 즉시 반영).
