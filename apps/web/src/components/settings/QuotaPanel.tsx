...
// 최근 30일 일별 사용액 라인차트. useQuota 의 실 데이터만 사용 — /usage/me 계약(16-API-CONTRACT
// §12)이 date 단위로만 집계돼 모델별 breakdown 필드가 없어, "모델별 비용" 표는 계약 확장 없이는
// 재현 불가(신규 라우트/계약 변경은 이 태스크 범위 밖 — 격리 없이 합계로 근사).
...
export function QuotaPanel() {
  const { quota, daily, loading, error } = useQuota();
