...
Phase 1 (auth) 와 Phase 2 (sessions/messages) 가 모두 적용된 _이후_ 의 onboarding test:
...
| 10    | 상용 수준 agentic chat UX — 툴콜/HITL/인용/아티팩트 end-to-end 렌더 + a11y(WCAG AA) ([19-UIUX-UPGRADE.md](19-UIUX-UPGRADE.md))                 |
| 11    | 멀티-LLM 선택 실반영 + universal tool-use 런타임 활성화(web_search/code_interpreter/병렬툴) ([20-MULTI-AGENT-TOOL.md](20-MULTI-AGENT-TOOL.md)) |
| 12    | 멀티에이전트 오케스트레이션(orchestrator-worker·DAG 병렬·handoff·MAST 가드·deep_research) ([20-MULTI-AGENT-TOOL.md](20-MULTI-AGENT-TOOL.md))   |
...
### Phase 10 (Commercial-Grade Agentic Chat UX, post-v1.0)

> 목적/원칙/태스크 상세 단일 출처: **[19-UIUX-UPGRADE.md](19-UIUX-UPGRADE.md)**. 여기는 팀×소유 요약. 태스크 = `feature_list.json` 의 `P10-*` (T6 17 + T2 6). sprint_key: `v1.0-S10-uiux`.
> 범위: A 기반(셸/사이드바/렌더/스트리밍/a11y) · B 에이전틱 라이브(툴콜/HITL/인용/아티팩트, **T6 렌더러 + T2 emit**) · C 컴포저(첨부/슬래시·멘션/모델피커) · D 대화관리·신뢰(프로젝트·메모리/편집·분기/공유/에러). 거버넌스(SSO·DLP·감사·evals)는 범위 밖(19 § 19.8).

| 팀                  | 입력 계약                                              | 소유 파일                                                                                                                                                                | 출력                                                                                                                                | 완료 조건                                                                                     |
| ------------------- | ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| **T6 Frontend**     | 19 § 19.5, 18-FRONTEND § 18.5.1, 14 § ChatEvent(동결)  | `apps/web/src/components/{layout,sessions,chat}/**`, `apps/web/src/components/artifacts/**`(T4 공동), `apps/web/src/app/(chat)/**`, `hooks/**`, `lib/citation-plugin.ts` | AppShell·사이드바·메시지렌더·스트리밍/a11y·ToolCall/HITL/Citation/Artifact 렌더러·컴포저(첨부/슬래시/피커)·편집분기·공유·에러토스트 | 동결 이벤트 shape 에 SSE-stub RED-first, web ≥ 60%, dev 브라우저 렌더 확인                    |
| **T2 Orchestrator** | 14 § ChatEvent/LLMProvider, 16 § /messages·hitl·resume | `apps/server/src/orchestrator/**`, `apps/server/src/tools/handlers/**`, `apps/server/src/routes/{sessions,messages}.ts`                                                  | 실 tool-execution 루프·HITL 브리지·citation/artifact_created emit·resume-stream·attachments 수용                                    | tool_use→tool_result 시퀀스·hitl pause/resume·citation/artifact emit·abort 관통, server ≥ 80% |
| **T3/T4**           | (P10 직접 태스크 없음)                                 | —                                                                                                                                                                        | 기존 retrieval·artifact-store 를 T2 핸들러가 호출                                                                                   | T2 핸들러가 기존 코드 호출(편집 필요 시 해당 태스크 격리)                                     |
| **T1/T5**           | —                                                      | —                                                                                                                                                                        | —                                                                                                                                   | (P10 무관)                                                                                    |

**Integration/순서**: 각 B 기능은 T2(emit) + T6(렌더) 쌍이 모두 완료돼야 end-to-end. 루프 우선순위 = `feature_list.json` P10 배열 순서(기반 → B 쌍 → C → D). 원격 반영(push/PR)은 사람이 수행.

### Phase 11 (Multi-LLM · Universal Tool 활성화, post-P10)

> 목적/원칙/태스크 상세 단일 출처: **[20-MULTI-AGENT-TOOL.md](20-MULTI-AGENT-TOOL.md)** § 20.5. 태스크 = `feature_list.json` `P11-*`. sprint_key: `v1.0-S11-tools`.
> 핵심: last-mile 배선(provider 가 tools forward + route 가 tools/model 주입)으로 tool-use 런타임 활성화 → 멀티-LLM 레지스트리/어댑터(OpenAI/Gemini)/라우터/failover → 병렬 tool + arg 검증 → **web_search(Tavily)·code_interpreter(E2B)** 내장 툴 + MCP 보안 하드닝. 전부 `apps/server` 내부(동결 `packages/interfaces` 불변, 새 계약은 격리).

| 팀                  | 입력 계약                                                           | 소유 파일                                                                                                                                                                   | 출력                                                                                                                                          | 완료 조건                                                                              |
| ------------------- | ------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| **T2 Orchestrator** | 14 § LLMProvider/ChatInput/AgentToolSpec/ChatEvent(동결), 20 § 20.5 | `orchestrator/{llm-provider-*,llm-provider-registry,model-router,llm-failover,tool-schema-codec}.ts`, `tools/{handlers,arg-validator,tool-router}.ts`, `routes/messages.ts` | provider tool forward, 라우트 tool/model 주입, 레지스트리·OpenAI/Gemini 어댑터·라우터·failover, 병렬 tool, web_search/code_interpreter 핸들러 | 실HTTP tool_use↔tool_result 왕복, 모델 선택 실반영, 병렬 allow/직렬 hitl, server ≥ 80% |
| **T1 Platform**     | 14 § SandboxTransport(동결), 12 § SSRF                              | `tools/sandbox/**`(E2B), `mcp/url-validator.ts`, `routes/config.ts`(신규 마운트+guard), `lib/tool-metrics.ts`                                                               | SandboxTransport E2B 구현+fake, MCP invoke SSRF 재검증+description 변경감지, GET /config, 툴 관측                                             | egress 차단, SSRF 방어, org.allowedModels 노출                                         |
| **T3 Knowledge**    | (기존 retrieval 재사용)                                             | `knowledge/**`(호출만)                                                                                                                                                      | web-result citation 페이로드 확정                                                                                                             | citation ChatEvent fan-out                                                             |

### Phase 12 (Multi-Agent Orchestration, post-P11)

> 단일 출처: **[20-MULTI-AGENT-TOOL.md](20-MULTI-AGENT-TOOL.md)** § 20.6. 태스크 = `P12-*`. sprint_key: `v1.0-S12-agents`. **P11(단일 에이전트 활성화) 검증 후 착수**(Anthropic "단순 먼저").
> 핵심: orchestrator-worker(격리 runTurn, 최종메시지만 반환) + 의존성 DAG 병렬(LLMCompiler/ReWOO) + handoff + evaluator-optimizer + MAST 신뢰성 가드 + 대규모 tool-router + **deep_research 툴**(plan→병렬검색→종합+인용). 고가치 태스크에만(토큰 ~15x). 새 ChatEvent 필요 항목은 격리.

| 팀                  | 입력 계약                              | 소유 파일                                                                                                              | 출력                                                                                                                | 완료 조건                                                      |
| ------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **T2 Orchestrator** | 20 § 20.6, MAST/LLMCompiler/ReWOO 참조 | `orchestrator/{orchestrator-worker,dag-planner,router,evaluator,mast-guards,deep-research}.ts`, `tools/tool-router.ts` | orchestrator-worker 조합, DAG 병렬, AbortSignal fan-out, handoff, evaluator-optimizer, MAST 4가드, deep_research 툴 | 병렬 fan-out+최종메시지만 반환, 취소 전파, MAST 실패재현 green |
| **T6 Frontend**     | 18 § 우패널, 동결 ChatEvent            | `apps/web/src/components/**`                                                                                           | 멀티에이전트 활동 UI(기존 tool_use 카드 범위)                                                                       | 새 ChatEvent 필요 시 격리                                      |
