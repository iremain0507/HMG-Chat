...
## 10.1 Agents (커스텀 워크스페이스 에이전트 — P22-T6-10, 계약 승인 C5)

기본 모델 + 시스템 프롬프트 + 도구/스킬/지식 스코프를 묶은 프리셋. 전부 org-scoped 이고,
`visibility="private"` 는 작성자에게만 보인다. 다른 org(또는 남의 private)의 리소스는
존재 자체를 숨기기 위해 403 이 아닌 **404** 를 반환한다.

### `POST /agents`

```
Request: {
  name: string,                    # org 안에서 유일 (UNIQUE (org_id, name))
  baseModel: string,               # organizations.allowedModels 중 하나
  description?: string,
  systemPrompt?: string,
  toolIds?: string[],              # AgentToolSpec.name
  skillIds?: string[],             # SkillSpec.id
  projectIds?: string[],           # 지식 스코프
  visibility?: "private"|"org"     # 기본 "private"
}
Response 201: { data: <Agent>, meta: { requestId } }
Response 400 INVALID_INPUT / 409 CONFLICT(이름 중복)
```

### `GET /agents`

```
Response 200: { data: Array<Agent>, meta: { requestId } }
자기 org + (visibility="org" 또는 본인이 만든 private) 만 포함
```

### `GET /agents/:id` — 200 | 404

### `PATCH /agents/:id` — POST body 의 부분집합, 200 | 400 | 404 | 409
...
export async function createSession(
  body: paths["/sessions"]["post"]["requestBody"],
) {
  // ... typed
}
```
...
export const Message = z.object({
  ...
  runStatus: z
    ...
    .optional(), // sessions_active_runs 와 join — assistant message 만 의미. user/tool 메시지는 미설정.
...
export const IndexStatus = z.enum([
  "pending",
  "parsing",
  "chunking",
  "embedding",
...
export const SkillSpec = z.object({
  ...
  assets: z
    .array(z.object({ filename: z.string(), s3Key: z.string() }))
    .optional(),
});
